...
public class CreateMaximumNumber {
    ...
    public int[] maxNumber(int[] nums1, int[] nums2, int k) {
        int[] rst = new int[k];
        recursive(nums1, nums2, 0, 0, 0, k, rst);
    ...
    public void recursive(int[] nums1, int[] nums2, int index1, int index2, int currK, int k, int[] tmpRst){
        if(k == currK){
...
//        System.out.println("index1=" + index1 + ", index2=" + index2 + ", currK=" + currK);
        Integer[] rst1 = findMaxNumber(nums1, index1, nums2.length-index2, k-currK);
        Integer[] rst2 = findMaxNumber(nums2, index2, nums1.length-index1, k-currK);
        if(rst1 == null){
            tmpRst[currK] = rst2[0];
            recursive(nums1, nums2, index1, rst2[1]+1, currK+1, k, tmpRst);
        } else if(rst2 == null){
            tmpRst[currK] = rst1[0];
            recursive(nums1, nums2, rst1[1] + 1, index2, currK+1, k, tmpRst);
    ...
    private Integer[] findMaxNumber(int[] nums, int startIdx, int otherLeft, int remainK) {
        if(startIdx >=nums.length){
            return null;
        }
        int mustLeft = remainK - otherLeft;
        int maxIndex = mustLeft > 0?nums.length-mustLeft:nums.length-1;
        int tmpMax = -1;
        int index = startIdx;
        ...
        return new Integer[]{tmpMax, index};
    }
    ...
    public static void main(String[] args) {
        ...
        k = 15;
        System.out.println(Arrays.toString(new CreateMaximumNumber().maxNumber(nums1, nums2, k)));
