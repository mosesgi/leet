...
public class CampusBikes {
    public int[] assignBikes(int[][] workers, int[][] bikes) {
        ...
        for(int i=0; i<workers.length; i++){
            for(int j=0; j<bikes.length; j++){
                list.add(new Element(i, j, workers[i], bikes[j]));
            }
        }
        Collections.sort(list, (o1, o2) -> o1.distance-o2.distance);
        int[] res = new int[workers.length];
        for(Element l : list){
            if(workerUsed[l.worker] || bikeUsed[l.bike]){
                continue;
            }
            workerUsed[l.worker] = true;
            bikeUsed[l.bike] = true;
            res[l.worker] = l.bike;
        }
        return res;
    }
    ...
    class Element{
        ...
        public Element(int worker, int bike, int[] workerPos, int[] bikePos){
            this.worker = worker;
            this.bike = bike;
            distance = calculateDistance(workerPos, bikePos);
        }
    }

    private int calculateDistance(int[] workerPos, int[] bikePos) {
        return Math.abs(bikePos[0] - workerPos[0]) + Math.abs(bikePos[1] - workerPos[1]);
    }
}
