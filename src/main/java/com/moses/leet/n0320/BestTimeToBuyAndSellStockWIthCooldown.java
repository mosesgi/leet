...
public class BestTimeToBuyAndSellStockWIthCooldown {
    Map<Integer, Integer> buyCache = new HashMap<>();
//    Map<String, Integer> sellCache = new HashMap<>();     not working
    ...
    public int maxProfitMyOwn(int[] prices) {
        return recursive(prices, 0, true, null);
    }

    private int recursive(int[] prices, int pos, boolean isEmpty, Integer buyPrice) {
        if(pos > prices.length-1){
            return 0;
        }

        if(isEmpty){
            if(buyCache.containsKey(pos)){
                return buyCache.get(pos);
            }
            //buy
            int b = recursive(prices, pos+1, false, prices[pos]);
            //not buy
            int nb = recursive(prices, pos+1, true, null);
            int max = Math.max(b, nb);
            buyCache.put(pos, max);
            return max;
        } else {
            String key = buyPrice + "_" + pos;
//            if(sellCache.containsKey(key)){
//                return sellCache.get(key);
//            }
            //sell
            int s = 0;
            if(prices[pos] - buyPrice > 0) {
                s = recursive(prices, pos + 2, true, null); //, profit + (prices[pos] - buyPrice)
                s += (prices[pos] - buyPrice);
            }
            //not sell
            int ns = recursive(prices, pos+1, isEmpty, buyPrice);
            if(prices[pos] - buyPrice > 0){
                int max = Math.max(s, ns);
                return max;
//                sellCache.put(key, max);
            } else {
//                sellCache.put(key, ns);
                return ns;
            }
        }
    }
