...
public class MaximumVacationDays {
    ...
    public int maxVacationDays(int[][] flights, int[][] days) {
        Integer[][] mem = new Integer[days.length][days[0].length];
        return dfs(flights, days, 0, 0, mem);
    }

    int dfs(int[][] flights, int[][] days, int city, int week, Integer[][] mem){
        if(week >= days[0].length){
            return 0;
        }

        if(mem[city][week] != null){
            return mem[city][week];
        }
        int min = days[city][week] + dfs(flights, days, city, week+1, mem);

        for(int j=0; j<flights[city].length; j++){
            if(flights[city][j] > 0){
                int tmp = days[j][week] + dfs(flights, days, j, week+1, mem);
